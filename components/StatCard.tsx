...
export const StatCard: React.FC<KpiData> = ({ label, value, trend, change }) => {
  return (
    <div className="bg-white/40 backdrop-blur-md border border-winter-100 p-5 sm:p-6 rounded-2xl shadow-sm hover:shadow-md transition-all duration-300 hover:translate-y-[-2px] w-full">
      <h3 className="text-sm font-medium text-winter-600 mb-2 truncate">{label}</h3>
      <div className="flex items-end justify-between gap-2">
        <span className="text-2xl sm:text-3xl font-semibold text-winter-900 tracking-tight truncate">{value}</span>
