...
export const FrostHeader: React.FC<FrostHeaderProps> = ({ pageTitle, isSidebarOpen = true, onToggleSidebar }) => {
  return (
    <header className="sticky top-0 z-40 w-full h-16 px-4 md:px-8 flex items-center justify-between bg-[#F5F7FA]/80 backdrop-blur-xl border-b border-winter-200/60 transition-all duration-300">
      <div className="flex items-center gap-3 md:gap-4">
        {/* Always show toggle if closed, or on mobile if open (handled by sidebar overlay) */}
        {!isSidebarOpen && (
          <button 
            onClick={onToggleSidebar}
            className="p-2 -ml-2 rounded-lg text-winter-600 hover:bg-winter-100 hover:text-winter-900 transition-all animate-in fade-in zoom-in duration-200"
            aria-label="Open Sidebar"
          >
            <PanelLeftOpen size={20} strokeWidth={1.5} />
          </button>
        )}
        <h1 className="text-lg font-medium text-winter-900 tracking-tight truncate">{pageTitle}</h1>
      </div>
      ...
      <div className="flex items-center gap-3 md:gap-6">
        ...
        <button className="text-winter-600 hover:text-winter-900 transition-colors p-1">
          <Search size={18} strokeWidth={1.5} />
        </button>
        <button className="text-winter-600 hover:text-winter-900 transition-colors relative p-1">
